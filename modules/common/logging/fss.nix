...
let
  ...
  setupScript = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      # Note: On VMs with virtiofs writeOnly shares, mkdir may work but chmod won't
      # (permissions are controlled by the host-side daemon)
      mkdir -p "$KEY_DIR"
...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    systemd.tmpfiles.rules = lib.optionals (config.ghaf.type == "host") [
